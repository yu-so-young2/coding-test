...
public class Main_BOJ_11505_구간곱구하기_G1 {
    // Segment Tree 클래스
    static class SegmentTree {
        ...
        SegmentTree(long[] arr){ // arr 에 해당하는 index tree 초기 생성
            ...
            for (int i = 0; i < indexTree.length; i++) {
                indexTree[i] = 1;
            }
        ...
        private void makeSubTree(int treeIdx, int left, int right) { // left, right: 현재 노드가 커버하는 인덱스 범위
            ...
            this.indexTree[treeIdx] = (this.indexTree[treeIdx*2] * this.indexTree[treeIdx*2+1])%1000000007;

        }

        // index tree 중간 인덱스 값 갱신 O(logN)
        public void update(int updateIdx, int updateValue) {
            updateSubTree(1, 1, leafCount, updateIdx, updateValue);
            data[updateIdx-1] = updateValue;
        }

        private void updateSubTree(int treeIdx, int left, int right, int updateIdx, int updateValue) {
            if(left <= updateIdx && updateIdx <= right) {
                this.indexTree[treeIdx] = updateValue;
                if(left==right) return;
            }

            int mid = (left+right)/2;
            if(updateIdx <= mid) {
                updateSubTree(treeIdx*2, left, mid, updateIdx, updateValue);
            }
            else {
                updateSubTree(treeIdx*2+1, mid+1, right, updateIdx, updateValue);
            }
            this.indexTree[treeIdx] = (this.indexTree[treeIdx*2] * this.indexTree[treeIdx*2+1])%1000000007;
        ...
        // 구간합 구하기 O(logN)
        public long getPartialMul(int left, int right) {
            return getSubPartialMul(1, 1, leafCount, left, right);

        }
        private long getSubPartialMul(int treeIdx, int left, int right, int tleft, int tright) {
            ...
            int mid = (left+right)/2;
            return (getSubPartialMul(treeIdx*2, left, mid,tleft,tright)*getSubPartialMul(treeIdx*2+1, mid+1, right,tleft,tright))%1000000007;
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < M+K; i++) {
            ...
            if(a == 1) { // tree update
                ...
                int b = Integer.parseInt(st.nextToken());
                int c = Integer.parseInt(st.nextToken());
                sb.append(segmentTree.getPartialMul(b, c)+"\n");
            }
        }
        System.out.print(sb);
